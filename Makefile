...
REPO_URL := https://github.com/toozej/go-find-goodwill

# Detect the OS and architecture
OS := $(shell uname -s)
ARCH := $(shell uname -m)
LATEST_RELEASE_URL := $(REPO_URL)/releases/latest/download/go-find-goodwill_$(OS)_$(ARCH).tar.gz
...
vet: ## Run `go vet` in Docker
	docker build --target vet -f $(CURDIR)/Dockerfile -t toozej/go-find-goodwill:latest . 

test: ## Run `go test` with race detection in Docker
	docker build --progress=plain --target test -f $(CURDIR)/Dockerfile -t toozej/go-find-goodwill:latest .

build: ## Build Docker image, including running tests
	docker build -f $(CURDIR)/Dockerfile -t toozej/go-find-goodwill:latest .

get-cosign-pub-key: ## Get go-find-goodwill Cosign public key from GitHub
	test -f $(CURDIR)/go-find-goodwill.pub || curl --silent https://raw.githubusercontent.com/toozej/go-find-goodwill/main/go-find-goodwill.pub -O

verify: get-cosign-pub-key ## Verify Docker image with Cosign
	cosign verify --key $(CURDIR)/go-find-goodwill.pub toozej/go-find-goodwill:latest

run: ## Run built Docker image
	docker run --rm --name go-find-goodwill --env-file $(CURDIR)/.env toozej/go-find-goodwill:latest
...
distroless-build: ## Build Docker image using distroless as final base
	docker build -f $(CURDIR)/Dockerfile.distroless -t toozej/go-find-goodwill:distroless . 

distroless-run: ## Run built Docker image using distroless as final base
	docker run --rm --name go-find-goodwill -v $(CURDIR)/config:/config toozej/go-find-goodwill:distroless

install: ## Install go-find-goodwill from latest GitHub release
	if command -v go; then \
			go install github.com/toozej/go-find-goodwill@latest ; \
	else \
			echo "Downloading go-find-goodwill binary for $(OS)-$(ARCH)..."; \
			mkdir -p $(CURDIR)/tmp; \
			curl --silent -L -o $(CURDIR)/tmp/go-find-goodwill.tgz $(LATEST_RELEASE_URL); \
			tar -xzf $(CURDIR)/tmp/go-find-goodwill.tgz -C $(CURDIR)/tmp/; \
			chmod +x $(CURDIR)/tmp/go-find-goodwill; \
			sudo mv $(CURDIR)/tmp/go-find-goodwill /usr/local/bin/go-find-goodwill; \
			rm -rf $(CURDIR)/tmp; \
	fi
...
local-release: local-test docker-login ## Release assets using locally installed golang toolchain and goreleaser
	if test -e $(CURDIR)/go-find-goodwill.key && test -e $(CURDIR)/.env; then \
		...
		echo "no cosign private key found at $(CURDIR)/go-find-goodwill.key. Cannot release."; \
	fi

local-sign: local-test ## Sign locally installed golang toolchain and cosign
	if test -e $(CURDIR)/go-find-goodwill.key && test -e $(CURDIR)/.env; then \
		export `cat $(CURDIR)/.env | xargs` && cosign sign-blob --key=$(CURDIR)/go-find-goodwill.key --output-signature=$(CURDIR)/go-find-goodwill.sig $(CURDIR)/out/go-find-goodwill; \
	else \
		echo "no cosign private key found at $(CURDIR)/go-find-goodwill.key. Cannot release."; \
	fi
...
local-verify: get-cosign-pub-key ## Verify locally compiled binary
	...
	cosign verify-blob --key $(CURDIR)/go-find-goodwill.pub --signature $(CURDIR)/go-find-goodwill.sig $(CURDIR)/out/go-find-goodwill

local-install: local-build local-verify ## Install compiled binary to local machine
	sudo cp $(CURDIR)/out/go-find-goodwill /usr/local/bin/go-find-goodwill
	sudo chmod 0755 /usr/local/bin/go-find-goodwill

upload-secrets-to-gh: ## Upload secrets from .env file to GitHub Actions Secrets + Dependabot
	$(CURDIR)/scripts/upload_secrets_to_github.sh go-find-goodwill 

upload-secrets-envfile-to-1pass: ## Upload secrets and .env file to 1Password
	$(CURDIR)/scripts/upload_secrets_to_1password secrets go-find-goodwill
	$(CURDIR)/scripts/upload_secrets_to_1password envfile go-find-goodwill
...
pre-commit-run: ## Run pre-commit hooks against all files
	...
	go-licenses report github.com/toozej/go-find-goodwill/cmd/go-find-goodwill
	govulncheck ./...
...
clean: ## Remove any locally compiled binaries and profiles
	rm -f $(CURDIR)/out/go-find-goodwill
