...
func TestLoadYAMLConfig(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Simple YAML configuration without environment overrides",
			yamlContent: `shopgoodwill:
  username: "testuser"
  password: "testpass"
  api_base_url: "https://yaml-api.shopgoodwill.com"
  max_retries: 4
  request_timeout: "45s"
  ...
  concurrency: 4`,
			expectedConfig: Config{
				ShopGoodwill: ShopGoodwillConfig{
					Username:       "testuser",
					Password:       "testpass",
					APIBaseURL:     "https://yaml-api.shopgoodwill.com",
					MaxRetries:     4,
					RequestTimeout: 45 * time.Second,
				},
				Search: SearchConfig{
				...
				Web: WebConfig{
					...
					StaticDir:    "web/static",      // default
					TemplateDir:  "web/templates",   // default
					ReadTimeout:  30 * time.Second,  // default
					WriteTimeout: 30 * time.Second,  // default
					IdleTimeout:  120 * time.Second, // default
				},
				Database: DatabaseConfig{
					Path:              "./goodwill.db",  // default
					MaxConnections:    10,               // default
					ConnectionTimeout: 30 * time.Second, // default
				},
				AntiBot: AntiBotConfig{
					UserAgent: UserAgentConfig{
						RotationEnabled:  false,     // default
						RotationInterval: time.Hour, // default
						RequestsPerUA:    20,        // default
						MinSuccessRate:   0.8,       // default
					},
					Timing: TimingConfig{
						BaseInterval:       15 * time.Minute, // default
						MinJitter:          2 * time.Minute,  // default
						MaxJitter:          5 * time.Minute,  // default
						HumanLikeVariation: false,            // default
					},
					Throttling: ThrottlingConfig{
						RequestsPerMinute: 0, // default
						BurstLimit:        0, // default
					},
					Retry: RetryConfig{
					...
					Circuit: CircuitConfig{
						...
						ResetTimeout:     5 * time.Minute,  // default
					},
				},
				Logging: LoggingConfig{
					Level:      "info", // default
		...
		{
			name: "YAML file with defaults applied",
			yamlContent: `shopgoodwill:
  username: "testuser"
  password: "testpass"`,
			expectedConfig: Config{
				ShopGoodwill: ShopGoodwillConfig{
					Username:       "testuser",
					Password:       "testpass",
					APIBaseURL:     "https://api.shopgoodwill.com", // default
					MaxRetries:     3,                              // default
					RequestTimeout: 30 * time.Second,               // default
				},
				Search: SearchConfig{
					IntervalMinutes:           15,    // default
					MinInterval:               5,     // default
					MaxInterval:               1440,  // default
					NotificationThresholdDays: 1,     // default
					EnableRegexSearches:       false, // default
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Set mock environment variables (these should override YAML)
			for key, value := range tt.mockEnv {
				os.Setenv(key, value)
			}
			defer func() {
			...
			// Call function
			conf, err := LoadYAMLConfig(yamlPath)
			if tt.expectError {
				if err == nil {
			...
			if conf.AntiBot.Timing.BaseInterval != tt.expectedConfig.AntiBot.Timing.BaseInterval {
				t.Errorf("AntiBot.Timing.BaseInterval: expected %v, got %v", tt.expectedConfig.AntiBot.Timing.BaseInterval, conf.AntiBot.Timing.BaseInterval)
			}
			if conf.AntiBot.Timing.MinJitter != tt.expectedConfig.AntiBot.Timing.MinJitter {
				t.Errorf("AntiBot.Timing.MinJitter: expected %v, got %v", tt.expectedConfig.AntiBot.Timing.MinJitter, conf.AntiBot.Timing.MinJitter)
			}
			if conf.AntiBot.Timing.MaxJitter != tt.expectedConfig.AntiBot.Timing.MaxJitter {
				t.Errorf("AntiBot.Timing.MaxJitter: expected %v, got %v", tt.expectedConfig.AntiBot.Timing.MaxJitter, conf.AntiBot.Timing.MaxJitter)
			}

			if conf.AntiBot.Retry.MaxRetries != tt.expectedConfig.AntiBot.Retry.MaxRetries {
				t.Errorf("AntiBot.Retry.MaxRetries: expected %d, got %d", tt.expectedConfig.AntiBot.Retry.MaxRetries, conf.AntiBot.Retry.MaxRetries)
			}
			if conf.AntiBot.Retry.BaseDelay != tt.expectedConfig.AntiBot.Retry.BaseDelay {
				t.Errorf("AntiBot.Retry.BaseDelay: expected %v, got %v", tt.expectedConfig.AntiBot.Retry.BaseDelay, conf.AntiBot.Retry.BaseDelay)
			}
			if conf.AntiBot.Retry.MaxDelay != tt.expectedConfig.AntiBot.Retry.MaxDelay {
				t.Errorf("AntiBot.Retry.MaxDelay: expected %v, got %v", tt.expectedConfig.AntiBot.Retry.MaxDelay, conf.AntiBot.Retry.MaxDelay)
			}

			if conf.AntiBot.Circuit.FailureThreshold != tt.expectedConfig.AntiBot.Circuit.FailureThreshold {
				t.Errorf("AntiBot.Circuit.FailureThreshold: expected %d, got %d", tt.expectedConfig.AntiBot.Circuit.FailureThreshold, conf.AntiBot.Circuit.FailureThreshold)
			}
			if conf.AntiBot.Circuit.SuccessThreshold != tt.expectedConfig.AntiBot.Circuit.SuccessThreshold {
				t.Errorf("AntiBot.Circuit.SuccessThreshold: expected %d, got %d", tt.expectedConfig.AntiBot.Circuit.SuccessThreshold, conf.AntiBot.Circuit.SuccessThreshold)
			}
			if conf.AntiBot.Circuit.Timeout != tt.expectedConfig.AntiBot.Circuit.Timeout {
				t.Errorf("AntiBot.Circuit.Timeout: expected %v, got %v", tt.expectedConfig.AntiBot.Circuit.Timeout, conf.AntiBot.Circuit.Timeout)
			}
			if conf.AntiBot.Circuit.ResetTimeout != tt.expectedConfig.AntiBot.Circuit.ResetTimeout {
				t.Errorf("AntiBot.Circuit.ResetTimeout: expected %v, got %v", tt.expectedConfig.AntiBot.Circuit.ResetTimeout, conf.AntiBot.Circuit.ResetTimeout)
			}
