...
// NotifyFoundItems sends notifications for multiple found goodwill items
...
func (m *NotificationManager) NotifyFoundItems(ctx context.Context, items []*db.GormItem, search *db.GormSearch) error {
	if len(items) == 0 {
		return nil // No items to notify about
	}

	if m.condense {
		return m.sendCondensedNotification(ctx, items, search)
	}

	// Send individual notifications
	var errors []error
	for _, item := range items {
		if err := m.NotifyFound(ctx, item, search); err != nil {
			errors = append(errors, err)
		}
	}
	if len(errors) > 0 {
...
func (m *NotificationManager) sendCondensedNotification(ctx context.Context, items []*db.GormItem, search *db.GormSearch) error {
	if len(items) == 0 {
		return nil
	}
	...
	if len(items) == 1 {
		...
		item := items[0]
		subject = fmt.Sprintf("GFG - Found %s!", item.Title)
		var itemEndsAtStr string
		if item.EndsAt != nil {
			itemEndsAtStr = item.EndsAt.Format("2006-01-02 15:04:05")
		} else {
			itemEndsAtStr = "Unknown"
		}
		message.WriteString(fmt.Sprintf("Found %s at %s for $%.2f\nURL: %s\nCategory: %s\nEnds: %s",
			item.Title,
			item.Seller,
			item.CurrentPrice,
			item.URL,
		...
		subject = fmt.Sprintf("GFG - Found %d items!", len(items))
		message.WriteString(fmt.Sprintf("Found %d goodwill items:\n\n", len(items)))

		for i, item := range items {
			message.WriteString(fmt.Sprintf("%d. %s at %s for $%.2f\n",
				i+1,
				item.Title,
				item.Seller,
				item.CurrentPrice,
...
func (m *NotificationManager) NotifyHeartbeat(ctx context.Context) error {
	subject := "GFG - Heartbeat"
	message := "GFG is still running and searching"
